...
class ArrayExtender {
  ...
  public static function eachCons<T>(a: Array<T>, count: Int, cycle: Bool = false): Array<Array<T>> {
    if (count > a.length) return [];

    var ret = [];
    var cycleIdx = a.length - count + 1;
    var endIdx = cycle ? a.length : cycleIdx;

    for (i in 0...endIdx) {
      if (i >= cycleIdx) {
        ret.push(a.slice(i).concat(a.slice(0, count - (endIdx - i))));
      } else {
